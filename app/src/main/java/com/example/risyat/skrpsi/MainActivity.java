        ...
        runOnUiThread(new Runnable() {
            ...
            public void run() {
                for (int i = 0; i < root.getAsJsonArray().size(); i++) {
                    if(root.isJsonNull() || root.getAsJsonArray().get(i).isJsonNull() || root.getAsJsonArray().get(i).getAsJsonObject().get("id").isJsonNull()){
                        continue;
                    }
...
//    }
    ...
    protected void onResume() {
        ...
        for (int i = 0; i < Sensors.length; i++) {
            Sensors[i].setText(sp.getString(Data.SensorsText[i],Data.SensorStatusDefault[i]));
            System.out.println(sp.getString(Data.SensorsText[i],Data.SensorStatusDefault[i]));
            if (sp.getString(Data.SensorsText[i]+"color","white").equals("red") || sp.getString(Data.SensorsText[i]+"color","white") == "red" ){
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                    Sensors[i].setCompoundDrawableTintList(ColorStateList.valueOf(Color.RED));
                }
                Sensors[i].setTextColor(Color.RED);
            }else{
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                    Sensors[i].setCompoundDrawableTintList(ColorStateList.valueOf(Color.WHITE));
                }
                Sensors[i].setTextColor(Color.WHITE);
            }
            spe.putString(Data.SensorsText[i],Data.SensorStatusDefault[i]);
            spe.putString(Data.SensorsText[i]+"color","white");
            spe.commit();
        }
